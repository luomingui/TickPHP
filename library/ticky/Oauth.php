 ...
 * +----------------------------------------------------------------------
 * | SVN: $Id: Oauth.php 29529 2018-3-13 luomingui $
 * +----------------------------------------------------------------------
 * | Description：Oauth
 * +----------------------------------------------------------------------
 */
...
abstract class Oauth {
    ...
    public function __construct($token = null) {
        ...
        //下面三句是我添加的。
        $typeArr = explode('\\', $class);
        $typeLen = count($typeArr);
        $this->Type = $typeArr[$typeLen - 1];

        //获取应用配置
        $config = config("TICKY_SDK_" . strtoupper($this->Type));
        //debug($config);
        if (empty($config['APP_KEY']) || empty($config['APP_SECRET'])) {
            throw new Exception('请配置您申请的APP_KEY和APP_SECRET第三方type： ' . strtoupper($this->Type));
        } else {
    ...
    public static function getInstance($type, $token = null) {
        $type = isset($config['type']) ? $config['type'] : 'qq';
        $class = false !== strpos($type, '\\') ? $type : '\\ticky\\oauth\\' . ucwords($type);
